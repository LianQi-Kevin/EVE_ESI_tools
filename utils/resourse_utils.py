...
# 应用ID
CLINT_ID = "68dfba8597464c02928ca96793b7690f"
# 应用申请信息列表
SCOPE = "publicData " \
...
# 获取可刷新令牌
def get_refresh_token(client_id=CLINT_ID, scope=SCOPE, token_path="token"):
    # 尝试创建数据输出文件夹
...
def refresh_token_post(client_id, scope, token_path, name):
    ...
    res = requests.post(
        ...
        data=form_values,
        headers=headers,
        proxies={"http": "http://127.0.0.1:52539", "https": "http://127.0.0.1:52539",
                 "socks5": "http://127.0.0.1:52539"}
...
def translate_NPC_corpID(NPC_corp_ID, Language):
    # npcCorporations.yaml from https://developers.eveonline.com/resource
    with codecs.open("sde_resource/npcCorporations.yaml", "r", "utf-8") as corp_yaml:
